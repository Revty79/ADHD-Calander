...
### Exact Time Is Available Without Hiding Scheduling Semantics

Decision: Planned task editing offers an optional exact-time shortcut, and the
scheduling screen offers direct date/time placement alongside generated
suggestions. Choosing an exact time truthfully changes the task to Scheduled.
Android scheduling confirmation can save up to five reminder choices through
the same repository and notification synchronization path.

Reason: A user who already knows the intended time should not have to infer that
they must change modes or accept a generated suggestion. Keeping the stored
state Scheduled prevents exact commitments from being mislabeled as soft plans,
while explicit confirmation and reminder persistence satisfy the global
functional definition of done.

### Android Tabs Use Visible Text Badges And Strong Selection

Decision: Give every Android bottom tab a dependency-free text badge, taller
touch area, persistent label, and visible selected background.

Reason: Six small labels were difficult to distinguish on a real device. Text
badges avoid unsupported-glyph failures while keeping every destination visible
and accessible without adding an icon package.

### Android Native Project Is Intentionally Tracked

Decision: Keep the generated `android/` project under version control and treat
it as the Android build source of truth. Changes to native fields in `app.json`
must also be applied to the tracked Android project through Expo Prebuild or an
equivalent reviewed native change. Disable Expo Doctor's unsynced app-config
check because this workflow intentionally retains the native project.

Reason: Android builds and native notification testing depend on the committed
project. Removing it merely to satisfy the generic CNG check would discard the
approved native build setup; documenting the synchronization requirement keeps
the warning from being hidden without an explicit engineering decision.

### Planned Time Intent Is Semantic And Soft

Decision: Store `anytime`, `morning`, `afternoon`, or `evening` on dated tasks
instead of inventing clock times. Flexible tasks store null. Planned tasks show
the preference; Scheduled tasks keep it only so editing back to Planned can
retain intent. Morning is 06:00-12:00, Afternoon is 12:00-17:00, and Evening is
17:00-21:00 local time.

Reason: A user can express useful intent without making a false exact-time
commitment. The scheduler ranks a full preferred-period fit before fallback
times within the same load band, while all fixed-event, buffer, planning-hour,
deadline, duration, and daily-limit constraints remain authoritative.

### Planning Labels Match Stored Meaning Everywhere

Decision: Use Flexible only for tasks with no date, Planned for dated tasks with
no exact time, and Scheduled for dated tasks with an exact time. Today and
Calendar use the same categories; Planned tasks do not claim scheduled minutes.

Reason: Calling a dated task Flexible or an exact-time task Planned made the UI
contradict task editing and persistence, increasing the executive-function cost
of understanding the plan.

### Fixed-Event Creation Uses Selection Controls

Decision: Native event creation uses date and time pickers plus either an end
time picker or an explicit duration choice. Web uses semantic browser date/time
inputs and the same duration choices. Multiple native reminders remain wired;
web continues to state that browser notification delivery is unsupported.

Reason: Storage formats are not user input formats, and mutually exclusive
timing controls prevent malformed combinations without pretending unsupported
